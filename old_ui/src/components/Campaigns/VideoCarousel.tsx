    ...
    <div className="relative w-full h-screen bg-white flex items-center justify-center overflow-hidden">
      ...
      <AnimatePresence initial={false}>
        {videos.map((video, index) => (
          <motion.div
            key={video.id}
            className={`absolute w-[80%] h-[70%] bg-black bg-opacity-60 rounded-2xl shadow-2xl shadow-white flex items-center justify-center overflow-hidden
      ...
      <button
        ...
        className="absolute left-3 sm:left-8 z-30 text-green-300 text-6xl opacity-50 hover:opacity-100 transition-opacity duration-300"
      >
        &#8249;
      </button>
      <button
        ...
        className="absolute right-3 sm:right-8 z-30 text-green-300 text-6xl opacity-50 hover:opacity-100 transition-opacity duration-300"
      >
        &#8250;
      </button>
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-2">
        {videos.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full ${index === currentIndex ? 'bg-green-300' : 'bg-green-200'} transition-colors duration-300`}
